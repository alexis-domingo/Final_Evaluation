...
require_once 'connect.php'; //! Wrong name file

$order = '';
if(isset($_GET['order']) && isset($_GET['column'])){ //! Missing )
...
if(!empty($_POST)){
	foreach($_POST as $key => $value){
		$post[$key] = strip_tags(trim($value)); //! Delete one )
		}
	...
	if(strlen($post['firstname'] < 3)) {
		...
		// error = 0 = insertion user
			...
			$insertUser->bindValue(':firstname', $post['fistname']); //! Missing ;
